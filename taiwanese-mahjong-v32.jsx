...
// Find upgrade kong: player has an exposed pong and just drew the 4th matching tile
function findUpgradeKong(drawnTile, revealedSets) {
...
function aiCanClaim(hand, discardedTile, revealedSets = [], discardPile = []) {
  ...
  if (canWinWithTile(hand, discardedTile).win) return { type: "win" };
...
Give specific, actionable advice. Keep responses under 180 words.`;
  ...
  const handleAITurn = useCallback(() => {
    setGame(prev => {
      ...
      if (!drawn.isFlower) {
        ...
        for (let q = 1; q <= 3; q++) {
          const claimer = (p + q) % 4;
          if (claimer === 0) break; // human gets offered claim via UI, stop here
          const winCheck = canWinWithTile(newGame.hands[claimer], discard);
          if (winCheck.win) {
  ...
  const upgradeKongOption = (isMyTurn && drawnTile) ? findUpgradeKong(drawnTile, revealed[0]) : null;
