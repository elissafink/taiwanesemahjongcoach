...
function tileParts(tile) {
  ...
  // Tile face always uses neutral labels (visual only, not affected by mode)
  const suitLabel = { man: "Crk", bam: "Bam", circle: "Cir" }[tile.suit];
...
export default function MahjongApp() {
  ...
  // Mode-bound label helper — use tl() everywhere instead of tileLabel() directly
  const tl = (tile) => tl(tile, mode);
...
Give specific, actionable advice. Keep responses under 150 words.`;
  ...
  const handleAITurn = useCallback(() => {
    setGame(prev => {
      ...
      if (!drawn.isFlower) {
        ...
        const log = `${newGame.playerWinds[p]} discards ${tl(discard)}`;
  ...
  const handleDraw = useCallback(() => {
    setGame(prev => {
      ...
      const log = drawn
        ? `You drew ${tl(drawn)}${winCheck.win ? " — You can win! 🎉" : ""}`
        : "No tiles left!";
  ...
  const handleDiscard = useCallback((tile) => {
    setGame(prev => {
      ...
      for (let p = 1; p < 4; p++) {
        ...
        if (winCheck.win) {
          return {
            ...
            winner: p,
            phase: "won",
            gameLog: [...prev.gameLog.slice(-20), `You discarded ${tl(tile)}. Player ${p+1} (${prev.playerWinds[p]}) wins on your discard! 😱`]
          };
        }
        ...
        if (claim && Math.random() > 0.4) {
          ...
          return {
            ...
            drawnTile: null,
            currentPlayer: (p+1) % 4,
            gameLog: [...prev.gameLog.slice(-20), `You discarded ${tl(tile)}. ${prev.playerWinds[p]} claims ${claim.type.toUpperCase()} and discards ${tl(discard)}.`]
          };
        }
      }
      ...
      return {
        ...
        drawnTile: null,
        currentPlayer: 1,
        gameLog: [...prev.gameLog.slice(-20), `You discarded ${tl(tile)}`],
        claimOptions: null,
      };
    });
    setSelectedTile(null);
  ...
  const handleClaim = useCallback((option) => {
    ...
    if (option.type === "win") {
      ...
      setGame(prev => ({
        ...
        gameLog: [...prev.gameLog.slice(-20), `You win by claiming ${tl(discardedTile)}! 🎉`]
      }));
      setSelectedTile(null);
      return;
    }
    ...
    if (option.type === "pong") {
      ...
      setGame(prev => {
        ...
        return {
          ...
          currentPlayer: 0,
          gameLog: [...prev.gameLog.slice(-20), `You PONG ${tl(discardedTile)}! Now discard a tile.`]
        };
      });
      setSelectedTile(null);
      return;
      ...
      if (chowOptions.length === 1) {
        ...
        setGame(prev => {
          ...
          return {
            ...
            drawnTile: discardedTile,
            currentPlayer: 0,
            gameLog: [...prev.gameLog.slice(-20), `You CHOW ${tl(discardedTile)}! Now discard a tile.`]
          };
        });
        setSelectedTile(null);
      } else {
