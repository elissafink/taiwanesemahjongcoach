...
// mode: "clean" | "cabrona"
function suitAbbr(suit, mode) {
  if (mode === "cabrona") {
    return { man: "Crk", bam: "Pnis", circle: "Hole", wind: "Wind", dragon: "Drgn", flower: "dFlw" }[suit] || suit;
  }
  return { man: "Crk", bam: "Bam", circle: "Cir", wind: "Wind", dragon: "Drgn", flower: "Flwr" }[suit] || suit;
}

function tileLabel(tile, mode = "clean") {
  if (!tile) return "?";
  if (tile.isFlower) return `Flwr${tile.value}`;
  if (tile.suit === "wind") return { East: "E-Wind", South: "S-Wind", West: "W-Wind", North: "N-Wind" }[tile.value];
  if (tile.suit === "dragon") return { Red: "Red Drgn", Green: "Grn Drgn", White: "Wht Drgn" }[tile.value];
  return `${tile.value}${suitAbbr(tile.suit, mode)}`;
}

// Returns a two-line display for inside a tile: top symbol, bottom label
function tileParts(tile, mode = "clean") {
  ...
  // Suit label on tile face is mode-aware
  const suitLabel = suitAbbr(tile.suit, mode);
...
// Find concealed kong opportunities: groups of 4 identical tiles in hand
...
// Find upgrade kong: player has an exposed pong and the drawn tile is the 4th matching tile
// drawnTile must actually be present in the concealed hand (not just a post-claim marker)
function findUpgradeKong(drawnTile, revealedSets, concealedHand) {
  if (!drawnTile) return null;
  // Verify the drawn tile is actually in the concealed hand
  const inHand = concealedHand.some(t => t.id === drawnTile.id);
  if (!inHand) return null;
  return revealedSets.find(s => s.type === "pong" && isPair(s.tiles[0], drawnTile)) || null;
}
...
// canWin and canWinWithTile defined below with canWinWithTile

// Analyze hand for coaching
function analyzeHand(hand, discardPile, revealedSets, allPlayerDiscards, mode = "clean") {
  ...
  if (needToDiscard) {
    if (safeDiscards.length > 0) {
      ...
      reasoning = `${tileLabel(bestDiscard, mode)} is isolated in your hand AND has already been discarded by others — very safe to throw.`;
      ...
      if (bestDiscard.suit === "wind" || bestDiscard.suit === "dragon") {
        reasoning = `${tileLabel(bestDiscard, mode)} is isolated — no pair, triplet, or sequence connections. Honor tiles can only form triplets, not sequences, making them risky to hold without a pair.`;
      } else if (bestDiscard.value === 1 || bestDiscard.value === 9) {
        reasoning = `${tileLabel(bestDiscard, mode)} is an isolated edge tile with no useful neighbors. Ones and nines connect on only one side, so they're harder to complete than middle tiles.`;
      } else {
        reasoning = `${tileLabel(bestDiscard, mode)} has no useful connections — its nearest same-suit tile is too far away to form a sequence or pair. Shed it to improve your hand shape.`;
      }
      ...
      if (isDead) {
        reasoning = `${tileLabel(bestDiscard, mode)}'s sequence completion tile has been fully discarded — that run is dead. Break this partial and keep stronger connections.`;
      } else if (Math.abs(weakest.tiles[0].value - weakest.tiles[1].value) === 2) {
        reasoning = `${tileLabel(bestDiscard, mode)} is part of a gap sequence (needs the middle tile). Gap waits are harder to fill than connected pairs. Consider breaking this partial if you have a stronger path.`;
      } else {
        reasoning = `${tileLabel(bestDiscard, mode)} is in your weakest partial sequence. Your hand has decent shape overall — this is a judgment call about which suit to commit to.`;
      }
      ...
      if (honorPair) {
        ...
        reasoning = `Your hand is well-connected. If you need to release a tile, consider breaking the ${tileLabel(honorPair[0], mode)} pair — honor pairs are worth less than suit sequences toward a faster win, unless you're building toward All Triplets.`;
      } else {
...
// AI discard logic — smart: uses full hand analysis including revealed sets and discard pile
function aiDiscard(hand, revealedSets = [], discardPile = []) {
  const playable = hand.filter(t => !t.isFlower);
  if (playable.length === 0) return hand[0];

  // Count how many of each tile key are left (not yet discarded)
  // Used to assess whether a needed tile is still drawable
  const discardedCounts = {};
  for (const t of discardPile) {
    const key = `${t.suit}-${t.value}`;
    discardedCounts[key] = (discardedCounts[key] || 0) + 1;
  }
  const isDead = (suit, value) => (discardedCounts[`${suit}-${value}`] || 0) >= 4;

  // Score each tile as a discard candidate — higher = better to discard
  function discardScore(tile, remaining) {
    let score = 0;
    const { suit, value } = tile;

    // Honor tiles (winds/dragons) — good to discard unless in a pair/triplet
    if (suit === "wind" || suit === "dragon") {
      const matches = remaining.filter(t => isPair(t, tile)).length;
      if (matches === 0) score += 30; // isolated honor
      else if (matches === 1) score += 10; // honor pair — keepable but costly
      return score;
    }

    // Count connections: tiles that form sequences or pairs with this tile
    const pairs = remaining.filter(t => t.id !== tile.id && isPair(t, tile)).length;
    const seqLeft  = remaining.filter(t => t.suit === suit && t.value === value - 1).length;
    const seqLeft2 = remaining.filter(t => t.suit === suit && t.value === value - 2).length;
    const seqRight = remaining.filter(t => t.suit === suit && t.value === value + 1).length;
    const seqRight2= remaining.filter(t => t.suit === suit && t.value === value + 2).length;

    const connections = pairs * 3 + seqLeft + seqLeft2 + seqRight + seqRight2;
    score -= connections * 4; // more connected = less desirable to discard

    // Edge tiles (1, 9) are harder to use in sequences
    if (value === 1 || value === 9) score += 5;
    // Near-edge (2, 8) — slightly weaker
    if (value === 2 || value === 8) score += 2;

    // If needed tiles are dead (all discarded), the connection is worthless
    if (seqLeft && isDead(suit, value - 1)) score += 6;
    if (seqRight && isDead(suit, value + 1)) score += 6;

    return score;
  }

  // Score all tiles, pick the one with highest discard score
  const scored = playable.map(tile => {
    const remaining = playable.filter(t => t.id !== tile.id);
    return { tile, score: discardScore(tile, remaining) };
  });
...
// Check if AI hand is tenpai (one tile away from winning)
function isTenpai(hand, revealedSets = []) {
  const playable = hand.filter(t => !t.isFlower);
  if (playable.length !== 13) return false;
  // Try all possible completing tiles
  const suits = ["man", "bam", "circle"];
  for (const s of suits) {
    for (let v = 1; v <= 9; v++) {
      const test = [...playable, { suit: s, value: v, id: "test", isFlower: false }];
      if (findWinningGroups(test) || isAllPairs(test)) return true;
    }
  }
  for (const wind of ["East","South","West","North"]) {
    const test = [...playable, { suit: "wind", value: wind, id: "test", isFlower: false }];
    if (findWinningGroups(test)) return true;
  }
  for (const dragon of ["Red","Green","White"]) {
    const test = [...playable, { suit: "dragon", value: dragon, id: "test", isFlower: false }];
    if (findWinningGroups(test)) return true;
  }
  return false;
}

// Check if AI can claim a discard — strategic version
// Only claims when it materially improves the hand toward winning
function aiCanClaim(hand, discardedTile, revealedSets = [], discardPile = []) {
  const playable = hand.filter(t => !t.isFlower);

  // Always claim for win
  if (canWinWithTile(hand, discardedTile, revealedSets).win) return { type: "win" };

  // Count sets already have (revealed + those forming in hand)
  const totalSetsNeeded = 4 - revealedSets.length;

  // Check pong — only claim if triplet genuinely advances the hand
  const matches = playable.filter(t => isPair(t, discardedTile));
  if (matches.length >= 2) {
    // After ponging, hand loses 2 tiles. Check if the remaining hand
    // can still realistically form the remaining sets needed.
    const afterPong = playable.filter(t => t.id !== matches[0].id && t.id !== matches[1].id);
    const setsInHand = countCompleteSets(afterPong);
    // Claim pong if we're making progress (have sets forming) or if tenpai likely
    if (setsInHand >= totalSetsNeeded - 2 || totalSetsNeeded <= 2) {
      return { type: "pong", tiles: [matches[0], matches[1], discardedTile] };
    }
    // Also claim pong 60% of the time as tactical aggression
    if (Math.random() < 0.6) {
      return { type: "pong", tiles: [matches[0], matches[1], discardedTile] };
    }
  }

  // Check chow (only left player can chow — handled at call site)
  if (["man","bam","circle"].includes(discardedTile.suit)) {
    ...
    const chow = m2 && m1 ? [m2, m1, discardedTile]
               : m1 && p1 ? [m1, discardedTile, p1]
               : p1 && p2 ? [discardedTile, p1, p2] : null;
    if (chow && Math.random() < 0.5) {
      return { type: "chow", tiles: chow };
    }
  }
...
// Count complete sets (triplets + sequences) in a hand
function countCompleteSets(tiles) {
  const groups = findSets(sortHand(tiles));
  return groups ? groups.filter(g => g.type !== "pair").length : 0;
}

// Check if player can win with a specific tile
function canWin(concealedHand, revealedSets) {
  const tiles = concealedHand.filter(t => !t.isFlower);
  // Total effective tiles = concealed + revealed sets (each counts as 3)
  // Win condition: concealed tiles form (4 - revealedSets.length) sets + 1 pair
  // So concealed tile count should be: (4 - revealed) * 3 + 2
  const setsNeeded = 4 - (revealedSets ? revealedSets.length : 0);
  const expectedCount = setsNeeded * 3 + 2;
  if (tiles.length !== expectedCount) return { win: false };
  if (isAllPairs(tiles) && tiles.length === 14) return { win: true, type: "allPairs" };
  const groups = findWinningGroups(tiles);
  if (groups) return { win: true, type: "normal", groups };
  return { win: false };
}

function canWinWithTile(hand, tile, revealedSets = []) {
  const testHand = [...hand.filter(t => !t.isFlower), tile];
  return canWin(testHand, revealedSets);
}
...
function initGame() {
  ...
  // Keep flowers at end of deck so they're drawn during play, not dealt initially
  const mainDeck = deck.filter(t => !t.isFlower);
  const flowerTiles = shuffle(deck.filter(t => t.isFlower));
  deck = [...mainDeck, ...flowerTiles];
  
  // Deal 16 tiles to each player (from non-flower portion only)
  const hands = [[], [], [], []];
  const allDealt = mainDeck.splice(0, 64);
  for (let i = 0; i < 64; i++) hands[i % 4].push(allDealt[i]);
  
  // Remaining deck = undealt main tiles + flower tiles at end
  const remainingDeck = [...mainDeck, ...flowerTiles];
  ...
  return {
    ...
    flowers: [[], [], [], []],  // flowers per player — drawn during play
...
// TILE COMPONENT
// ============================================================
function TileComponent({ tile, selected, onClick, small, faceDown, dimmed, mode = "clean" }) {
  if (!tile) return null;
  const { top, bottom } = faceDown ? { top: "", bottom: "" } : tileParts(tile, mode);
...
export default function MahjongApp() {
  ...
  const [mode, setMode] = useState("clean"); // "clean" | "cabrona"
  const modeRef = useRef("clean");
  const chatEndRef = useRef(null);
  const aiTimerRef = useRef(null);

  // Keep modeRef in sync — lets useCallback closures read current mode without stale deps
  useEffect(() => { modeRef.current = mode; }, [mode]);

  // Mode-bound label helper for JSX — always reads current mode
  const tl = (tile) => tileLabel(tile, mode);
  // For use inside useCallback closures — reads from ref to avoid stale closure
  const tlRef = (tile) => tileLabel(tile, modeRef.current);
  ...
  const sendChatMessage = async (question) => {
    ...
    // Use playerHand (sorted, includes drawn tile) not raw hands[0]
    const { revealed, discardPile, discardPiles, playerWinds, flowers, drawnTile, claimOptions } = game;
    const myHand = playerHand.filter(t => !t.isFlower).map(t => tl(t)).join(", ");
    const myRevealed = revealed[0].map(s => `${s.type.toUpperCase()}(${s.tiles.map(t=>tl(t)).join("-")})`).join(", ") || "none";
    const myFlowers = flowers[0].map(t => tl(t)).join(", ") || "none";
    const centerDiscards = discardPile.slice(-30).map(t => tl(t)).join(", ") || "none";
    const opponentInfo = [1,2,3].map(p => {
      const exposed = revealed[p].map(s => `${s.type.toUpperCase()}(${s.tiles.map(t=>tl(t)).join("-")})`).join(", ") || "none";
      const discards = discardPiles[p].map(t => tl(t)).join(", ") || "none";
      return `  ${playerWinds[p]}: exposed sets: ${exposed} | discards: ${discards}`;
    }).join("\n");

    const handCount = playerHand.filter(t => !t.isFlower).length;
    const needsDiscard = drawnTile !== null;

    // Describe the current claim decision if one is pending
    let claimSituation = "none";
    if (claimOptions) {
      const tile = tl(claimOptions.discardedTile);
      const from = playerWinds[claimOptions.fromPlayer];
      const opts = claimOptions.options.map(o => o.type.toUpperCase()).join(", ");
      claimSituation = `${from} just discarded ${tile}. Player is being offered: ${opts}. Player must decide NOW whether to claim this tile.`;
    }

    // Suit names depend on current mode
    const suitKey = mode === "cabrona"
      ? "Crk = Characters (e.g. 7Crk), Pnis = Bamboo (e.g. 3Pnis), Hole = Circles (e.g. 5Hole)"
      : "Crk = Characters (e.g. 7Crk), Bam = Bamboo (e.g. 3Bam), Cir = Circles (e.g. 5Cir)";

    const systemPrompt = `You are an expert Taiwanese Mahjong coach. Answer the player's question concisely and clearly. Always base your advice ONLY on the exact tiles listed — do not infer, assume, or add tiles that aren't shown.

TILE NOTATION USED IN THIS GAME:
${suitKey}
Wind tiles: E-Wind, S-Wind, W-Wind, N-Wind
Dragon tiles: Red Drgn, Grn Drgn, Wht Drgn
Flower tiles: Flwr1–Flwr8

CURRENT GAME STATE:
- My concealed hand (${handCount} tiles): ${myHand}
- My exposed sets: ${myRevealed}
- My flowers: ${myFlowers}
- Currently needs to discard: ${needsDiscard ? "YES — player just drew and must discard 1 tile" : "NO — waiting to draw"}
- Active claim decision: ${claimSituation}
- Recent discards (center pile): ${centerDiscards}
- Opponents:
${opponentInfo}

IMPORTANT RULES FOR YOUR ANALYSIS:
- ${handCount} tiles in hand is ${needsDiscard ? "CORRECT and expected — player drew and now must discard 1" : "correct for waiting state"}
- Never say the hand count is wrong or unusual
- Only reference tiles that are explicitly listed above — do not hallucinate extra tiles
- Sets needed to win: 4 sets (sequence of 3 same-suit consecutive OR triplet of 3 identical) + 1 pair
- Special winning hand: 7 pairs
- Chow = claim sequence from left player's discard only. Pong/Kong = claim from any player's discard.
- If "Active claim decision" is not "none", prioritize answering whether to take that claim.

Give specific, actionable advice. Keep responses under 180 words.`;
  ...
  const claimAdvice = (() => {
    ...
    if (hasWin) return { recommendation: "win", reason: `You can win right now by claiming ${tl(discardedTile)}! Declare Mahjong! 🏆` };
    ...
    if (hasPong && !hasChow) {
      ...
      if (existingPongs >= 2) {
        return { recommendation: "pong", reason: `PONG ${tl(discardedTile)}. You're building an all-triplets hand — keep the triplets coming!` };
      }
      return { recommendation: "pong", reason: `PONG ${tl(discardedTile)} to claim a triplet. This exposes your hand but locks in a completed set.` };
    }
    ...
    if (hasChow && !hasPong) {
      ...
      return { recommendation: "chow", reason: `CHOW ${tl(discardedTile)} to complete a sequence (${bestChow ? bestChow.map(t=>tl(t)).join("-") : "sequence"}). This skips a draw but locks your turn order. Skip if you're close to winning without it.` };
    }
    ...
    if (hasPong && hasChow) {
      ...
      const handAnalysis = analyzeHand(hand, game.discardPile, game.revealed[0], game.discardPiles, mode);
      const sequenceLeaning = handAnalysis.partials.length > handAnalysis.pairs.length;

      if (isHonor) {
        return { recommendation: "pong", reason: `PONG ${tl(discardedTile)}. Honor tiles can't form sequences, so Pong is the stronger claim here. It completes a full set immediately.` };
      }
      if (existingPongs >= 2) {
        return { recommendation: "pong", reason: `PONG ${tl(discardedTile)}. You're already triplet-heavy — stay consistent and build toward All Triplets (對對胡).` };
      }
      if (sequenceLeaning) {
        const bestChow = chowOpts[0];
        return { recommendation: "chow", reason: `CHOW ${tl(discardedTile)} (${bestChow ? bestChow.map(t=>tl(t)).join("-") : "sequence"}). Your hand leans toward sequences. Chow completes a run and keeps your strategy consistent. Only Pong if you want to pivot to triplets.` };
      }
      return { recommendation: "pong", reason: `PONG ${tl(discardedTile)}. Your hand has good pair potential. Pong locks in a complete set immediately versus Chow which only builds a partial run.` };
    }
  ...
  const handleAITurn = useCallback(() => {
    setGame(prev => {
      ...
      if (!drawn.isFlower) {
        ...
        const discard = aiDiscard(newGame.hands[p], newGame.revealed[p], newGame.discardPile);
        ...
        const log = `${newGame.playerWinds[p]} discards ${tlRef(discard)}`;
        newGame.gameLog = [...newGame.gameLog.slice(-20), log];

        // Check if other AI players can claim (pong/win only — chow only for immediate left)
        let claimed = false;
        for (let q = 1; q <= 3; q++) {
          const claimer = (p + q) % 4;
          if (claimer === 0) continue; // human gets offered claim via UI separately below
          const winCheck = canWinWithTile(newGame.hands[claimer], discard, newGame.revealed[claimer]);
          if (winCheck.win) {
            newGame.winner = claimer;
            newGame.phase = "won";
            newGame.gameLog = [...newGame.gameLog.slice(-20), `${newGame.playerWinds[claimer]} wins by claiming ${tlRef(discard)}! 🎉`];
            claimed = true;
            break;
          }
          const canChowAI = q === 1 && ["man","bam","circle"].includes(discard.suit);
          const aiClaim = aiCanClaim(newGame.hands[claimer], discard, newGame.revealed[claimer], newGame.discardPile);
          if (aiClaim && (aiClaim.type !== "chow" || canChowAI)) {
            const claimTiles = aiClaim.tiles || [];
            const tilesToRemove = claimTiles.filter(t => t.id !== discard.id);
            newGame.hands[claimer] = newGame.hands[claimer].filter(t => !tilesToRemove.some(r => r.id === t.id));
            newGame.revealed[claimer] = [...newGame.revealed[claimer], { type: aiClaim.type, tiles: claimTiles }];
            const aiDisc = aiDiscard(newGame.hands[claimer], newGame.revealed[claimer], newGame.discardPile);
            newGame.hands[claimer] = newGame.hands[claimer].filter(t => t.id !== aiDisc.id);
            newGame.discardPiles[claimer] = [...newGame.discardPiles[claimer], aiDisc];
            newGame.discardPile = [...newGame.discardPile, aiDisc];
            newGame.lastDiscard = aiDisc;
            newGame.lastDiscardPlayer = claimer;
            newGame.gameLog = [...newGame.gameLog.slice(-20), `${newGame.playerWinds[claimer]} ${aiClaim.type.toUpperCase()}s ${tlRef(discard)} and discards ${tlRef(aiDisc)}`];
            newGame.currentPlayer = (claimer + 1) % 4;
            claimed = true;
            break;
          }
        }
        if (claimed) { return newGame; }

        // Check if player 0 can claim
        // Chow only valid for the player immediately to the left of the discarder
        // Human (seat 0) can only chow from seat 3 (North), since play goes 0→1→2→3→0
        const playerCanChow = p === 3 && findChowOptions(newGame.hands[0], discard).length > 0;
        const playerCanPong = newGame.hands[0].filter(t => isPair(t, discard)).length >= 2;
        const playerCanKong = newGame.hands[0].filter(t => isPair(t, discard)).length >= 3;
        const playerWin = canWinWithTile(newGame.hands[0], discard, newGame.revealed[0]);

        const claimOpts = [];
        if (playerCanKong) claimOpts.push({ type: "kong" });
  ...
  const handleDraw = useCallback(() => {
    setGame(prev => {
      ...
      const winCheck = drawn ? canWinWithTile(prev.hands[0], drawn, prev.revealed[0]) : { win: false };
      const log = drawn
        ? `You drew ${tlRef(drawn)}${winCheck.win ? " — You can win! 🎉" : ""}`
        : "No tiles left!";
  ...
  const handleDiscard = useCallback((tile) => {
    setGame(prev => {
      ...
      for (let p = 1; p < 4; p++) {
        const winCheck = canWinWithTile(prev.hands[p], tile, prev.revealed[p]);
        const claim = aiCanClaim(prev.hands[p], tile, prev.revealed[p], newDiscardPile);
        ...
        if (winCheck.win) {
          return {
            ...
            winner: p,
            phase: "won",
            gameLog: [...prev.gameLog.slice(-20), `You discarded ${tlRef(tile)}. Player ${p+1} (${prev.playerWinds[p]}) wins on your discard! 😱`]
          };
        }

        // Chow is only valid for the player immediately to the left of the discarder (seat 0 discards → seat 1 can chow)
        const canChow = claim && claim.type === "chow" && p === 1; // seat 1 is left of seat 0
        const validClaim = claim && (claim.type !== "chow" || canChow);

        if (validClaim && Math.random() > 0.4) {
          ...
          return {
            ...
            drawnTile: null,
            currentPlayer: (p+1) % 4,
            gameLog: [...prev.gameLog.slice(-20), `You discarded ${tlRef(tile)}. ${prev.playerWinds[p]} claims ${claim.type.toUpperCase()} and discards ${tlRef(discard)}.`]
          };
        }
      }
      ...
      return {
        ...
        drawnTile: null,
        currentPlayer: 1,
        gameLog: [...prev.gameLog.slice(-20), `You discarded ${tlRef(tile)}`],
        claimOptions: null,
      };
    });
    setSelectedTile(null);
  ...
  const handleClaim = useCallback((option) => {
    ...
    if (option.type === "win") {
      ...
      setGame(prev => ({
        ...
        gameLog: [...prev.gameLog.slice(-20), `You win by claiming ${tlRef(discardedTile)}! 🎉`]
      }));
      setSelectedTile(null);
      return;
    }
    ...
    if (option.type === "pong") {
      ...
      setGame(prev => {
        ...
        return {
          ...
          currentPlayer: 0,
          gameLog: [...prev.gameLog.slice(-20), `You PONG ${tlRef(discardedTile)}! Now discard a tile.`]
        };
      });
      setSelectedTile(null);
      return;
    ...
    if (option.type === "chow") {
      ...
      const winAlsoAvailable = game.claimOptions.options.some(o => o.type === "win");
      if (chowOptions.length === 1 && !winAlsoAvailable) {
        // Only one valid chow and no win available — auto-complete
        pushUndo(game, selectedTile);
        ...
        setGame(prev => {
          ...
          return {
            ...
            drawnTile: discardedTile,
            currentPlayer: 0,
            gameLog: [...prev.gameLog.slice(-20), `You CHOW ${tlRef(discardedTile)}! Now discard a tile.`]
          };
        });
        setSelectedTile(null);
      } else {
        // Multiple chow options OR win is also available — show picker so player can choose
        setChowPicker({ discardedTile, fromPlayer, chowOptions, canWin: winAlsoAvailable });
  ...
  // Draw a replacement tile after a kong (same as normal draw but skips the "need to draw" check)
  const drawReplacementTile = (prevGame) => {
    let newDeck = [...prevGame.deck];
    if (newDeck.length === 0) return { ...prevGame, phase: "draw" };
    let drawn = newDeck.shift();
    let newFlowers = [...prevGame.flowers[0]];
    while (drawn && drawn.isFlower && newDeck.length > 0) {
      newFlowers.push(drawn);
      drawn = newDeck.shift();
    }
    if (drawn && drawn.isFlower) return { ...prevGame, deck: newDeck, flowers: prevGame.flowers.map((f,i) => i===0 ? newFlowers : f) };
    const newHand = drawn ? sortHand([...prevGame.hands[0], drawn]) : prevGame.hands[0];
    return {
      ...prevGame,
      deck: newDeck,
      hands: prevGame.hands.map((h,i) => i===0 ? newHand : h),
      flowers: prevGame.flowers.map((f,i) => i===0 ? newFlowers : f),
      drawnTile: drawn || null,
    };
  };

  // Kong from hand (concealed): player has 4 identical tiles in concealed hand
  // tileKey identifies which group to kong (suit-value), computed inside setGame to avoid stale closure
  const handleConcealedKong = useCallback((tileKey) => {
    pushUndo(game, selectedTile);
    setGame(prev => {
      // Recompute kong tiles from current state to avoid stale closure mismatch
      const groups = {};
      for (const t of prev.hands[0].filter(t => !t.isFlower)) {
        const key = `${t.suit}-${t.value}`;
        if (!groups[key]) groups[key] = [];
        groups[key].push(t);
      }
      const kongTiles = groups[tileKey];
      if (!kongTiles || kongTiles.length < 4) return prev; // safety check
      const newHand = prev.hands[0].filter(t => !kongTiles.some(k => k.id === t.id));
      const withKong = {
        ...prev,
        hands: prev.hands.map((h,i) => i===0 ? newHand : h),
        revealed: prev.revealed.map((r,i) => i===0 ? [...r, { type: "kong", tiles: kongTiles, isConcealed: true }] : r),
        drawnTile: null,
        gameLog: [...prev.gameLog.slice(-20), `You declare a Concealed KONG (${tlRef(kongTiles[0])} ×4)! Drawing replacement tile.`]
      };
      return drawReplacementTile(withKong);
    });
    setSelectedTile(null);
  }, [game, selectedTile, pushUndo]);

  // Kong from discard (melded): player has 3 in hand, claims opponent's discard as 4th
  const handleMeldedKong = useCallback(() => {
    if (!game.claimOptions) return;
    const { discardedTile } = game.claimOptions;
    pushUndo(game, selectedTile);
    setGame(prev => {
      const matches = prev.hands[0].filter(t => isPair(t, discardedTile));
      const kongTiles = [...matches.slice(0,3), discardedTile];
      const tilesToRemove = matches.slice(0,3);
      const newHand = prev.hands[0].filter(t => !tilesToRemove.some(r => r.id === t.id));
      const withKong = {
        ...prev,
        hands: prev.hands.map((h,i) => i===0 ? newHand : h),
        revealed: prev.revealed.map((r,i) => i===0 ? [...r, { type: "kong", tiles: kongTiles, isConcealed: false }] : r),
        claimOptions: null,
        drawnTile: null,
        currentPlayer: 0,
        gameLog: [...prev.gameLog.slice(-20), `You claim a Melded KONG (${tlRef(discardedTile)} ×4)! Drawing replacement tile.`]
      };
      return drawReplacementTile(withKong);
    });
    setSelectedTile(null);
  ...
  // Upgrade kong: player has exposed pong, draws the 4th matching tile
  const handleUpgradeKong = useCallback((pongKey) => {
    if (!game.drawnTile) return;
    pushUndo(game, selectedTile);
    setGame(prev => {
      if (!prev.drawnTile) return prev;
      const pongSet = prev.revealed[0].find(s => s.type === "pong" && `${s.tiles[0].suit}-${s.tiles[0].value}` === pongKey);
      if (!pongSet) return prev;
      const newHand = prev.hands[0].filter(t => t.id !== prev.drawnTile.id);
      const upgradedSet = { type: "kong", tiles: [...pongSet.tiles, prev.drawnTile], isConcealed: false };
      const newRevealed = prev.revealed[0].map(s => s === pongSet ? upgradedSet : s);
      const withKong = {
        ...prev,
        hands: prev.hands.map((h,i) => i===0 ? newHand : h),
        revealed: prev.revealed.map((r,i) => i===0 ? newRevealed : r),
        drawnTile: null,
        gameLog: [...prev.gameLog.slice(-20), `You upgrade Pong → KONG (${tlRef(prev.drawnTile)} ×4)! Drawing replacement tile.`]
      };
      return drawReplacementTile(withKong);
    });
    setSelectedTile(null);
  ...
  // Kong opportunities available after draw
  const concealedKongOptions = (isMyTurn && drawnTile) ? findConcealedKongs(playerHand.filter(t => !t.isFlower)) : [];
  const upgradeKongOption = (isMyTurn && drawnTile) ? findUpgradeKong(drawnTile, revealed[0], playerHand.filter(t => !t.isFlower)) : null;
        ...
        <div style={{
          ...
          {chowPicker.canWin && (
            <button
              className="win-btn"
              onClick={() => {
                const dt = chowPicker.discardedTile;
                setChowPicker(null);
                pushUndo(game, selectedTile);
                setGame(prev => ({
                  ...prev,
                  ...
                  winner: 0,
                  phase: "won",
                  hands: prev.hands.map((h,i) => i===0 ? [...h, dt] : h),
                  gameLog: [...prev.gameLog.slice(-20), `You win by claiming ${tl(dt)}! 🎉`]
                }));
              }}
              style={{ fontSize: 14, padding: "10px 18px", marginBottom: 14, width: "100%" }}
            >
              🏆 WIN instead! (claim {tl(discardedTile)} as your winning tile)
            </button>
          )}
          <div style={{ fontSize: 12, color: "#8a7a5a", marginBottom: 16, lineHeight: 1.6 }}>
            The discarded tile is <span style={{ color: tileColor(discardedTile), fontWeight: "bold" }}>{tl(discardedTile)}</span>.
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  {combo.map((t, ti) => (
                    <div key={ti} style={{ position: "relative" }}>
                      <TileComponent tile={t} small mode={mode} />
    ...
    <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {coaching.bestDiscard ? (
            <div style={{ background: "rgba(212,168,67,0.08)", border: "1px solid rgba(212,168,67,0.25)", borderRadius: 6, padding: "10px 12px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                <TileComponent tile={coaching.bestDiscard} small mode={mode} />
                <div style={{ fontSize: 14, fontWeight: "bold", color: tileColor(coaching.bestDiscard) }}>{tl(coaching.bestDiscard)}</div>
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              {coaching.triplets.length > 0 && (
                <div style={{ fontSize: 12, color: COLORS.text }}>
                  ...
                  <span style={{ color: COLORS.textMuted }}>{coaching.triplets.map(t => tl(t[0])).join(", ")}</span>
                </div>
              )}
              {coaching.sequences.length > 0 && (
                <div style={{ fontSize: 12, color: COLORS.text }}>
                  <span style={{ color: "#22c55e" }}>✓ Sequences ({coaching.sequences.length}):</span>{" "}
                  <span style={{ color: COLORS.textMuted }}>{coaching.sequences.map(s => s.map(t=>tl(t)).join("-")).join(", ")}</span>
                </div>
              )}
              {coaching.pairs.length > 0 && (
                <div style={{ fontSize: 12, color: COLORS.text }}>
                  <span style={{ color: "#60a5fa" }}>Pairs ({coaching.pairs.length}):</span>{" "}
                  <span style={{ color: COLORS.textMuted }}>{coaching.pairs.map(p => tl(p[0])).join(", ")}</span>
                </div>
              )}
              {coaching.partials.length > 0 && (
                <div style={{ fontSize: 12, color: COLORS.text }}>
                  <span style={{ color: "#a78bfa" }}>Partials ({coaching.partials.length}):</span>{" "}
                  <span style={{ color: COLORS.textMuted }}>{coaching.partials.slice(0,4).map(p => `${tl(p.tiles[0])}-${tl(p.tiles[1])}`).join(", ")}</span>
                </div>
              )}
              {coaching.isolated.length > 0 && (
                <div style={{ fontSize: 12, color: COLORS.text }}>
                  <span style={{ color: "#f87171" }}>Isolated ({coaching.isolated.length}):</span>{" "}
                  <span style={{ color: COLORS.textMuted }}>{coaching.isolated.map(t => tl(t)).join(", ")}</span>
                </div>
              )}
              {coaching.safeDiscards.length > 0 && (
                <div style={{ fontSize: 12, color: COLORS.text }}>
                  <span style={{ color: "#22c55e" }}>Safe to discard:</span>{" "}
                  <span style={{ color: COLORS.textMuted }}>{coaching.safeDiscards.map(t => tl(t)).join(", ")} (already discarded by others)</span>
                </div>
              )}
            </div>
          </div>
      ...
      <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: "10px 12px", marginTop: "auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 3 }}>
          {[
            { label: `Crk — Characters`, color: "#f97316" },
            { label: `${mode === "cabrona" ? "Pnis" : "Bam"} — Bamboo`, color: "#4ade80" },
            { label: `${mode === "cabrona" ? "Hole" : "Cir"} — Circles`, color: "#60a5fa" },
            { label: "E/S/W/N — Winds", color: "#a78bfa" },
            { label: "Grn/Wht/Red — Dragons", color: "#fbbf24" },
            { label: `${mode === "cabrona" ? "dFlw" : "Flwr"} — Flowers`, color: "#22c55e" },
          ].map(({ label, color }) => (
    ...
    <div style={{
      ...
      <div style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
        <div style={{ fontSize: 10, color: currentPlayer === 2 ? COLORS.goldLight : COLORS.textMuted, fontFamily: "'Cinzel', serif", letterSpacing: 1 }}>
          {currentPlayer === 2 && <span className="pulsing">▼ </span>}WEST · {hands[2].filter(t=>!t.isFlower).length + revealed[2].reduce((s,r)=>s+3,0)}
          {flowers[2].length > 0 && <span style={{ color: "#22c55e", marginLeft: 6 }}>🌸×{flowers[2].length}</span>}
        </div>
        <div style={{ display: "flex", gap: 2, overflowX: "auto", overflowY: "hidden", width: "100%", paddingBottom: 2, flexWrap: "nowrap" }}>
          {revealed[2].map((set, si) => (
            <div key={si} style={{ display: "flex", gap: 1, flexShrink: 0, background: "rgba(212,168,67,0.1)", padding: "2px 3px", borderRadius: 3, border: "1px solid rgba(212,168,67,0.2)" }}>
              {set.tiles.map((t, ti) => <TileComponent key={ti} tile={t} small mode={mode} />)}
            </div>
          ))}
          {hands[2].filter(t=>!t.isFlower).slice(0, compact ? 5 : 9).map((_, i) => <TileComponent key={i} tile={_} faceDown small mode={mode} />)}
          {hands[2].filter(t=>!t.isFlower).length > (compact ? 5 : 9) && (
        ...
        {discardPiles[2].length > 0 && (
          <div style={{ display: "flex", gap: 2, overflowX: "auto", overflowY: "hidden", width: "100%", paddingBottom: 2, flexWrap: "nowrap" }}>
            {discardPiles[2].slice(-(compact ? 6 : 8)).map((t, i) => <TileComponent key={i} tile={t} small mode={mode} />)}
          </div>
        )}
      </div>

      {/* Middle row — fixed height, side players and center all same height */}
      <div style={{ display: "flex", gap: compact ? 4 : 8, alignItems: "stretch", height: compact ? 160 : 230 }}>

        {/* South — scrollable vertically, wider to show revealed sets */}
        <div style={{ width: compact ? 40 : 62, flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, overflowY: "auto", overflowX: "hidden" }}>
          <div style={{ fontSize: 9, color: currentPlayer === 1 ? COLORS.goldLight : COLORS.textMuted, fontFamily: "'Cinzel', serif", writingMode: "vertical-rl", marginBottom: 2 }}>
            {currentPlayer === 1 && "▶ "}SOUTH{flowers[1].length > 0 ? ` 🌸${flowers[1].length}` : ""}
          </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 1, justifyContent: "center" }}>
            {hands[1].filter(t=>!t.isFlower).slice(0, compact ? 4 : 5).map((_, i) => <TileComponent key={i} tile={_} faceDown small mode={mode} />)}
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 1, justifyContent: "center", marginTop: 2 }}>
            {discardPiles[1].slice(-3).map((t, i) => <TileComponent key={i} tile={t} small mode={mode} />)}
          </div>
        </div>

        {/* Center discard — fixed height, tiles fill top, log fills bottom */}
        <div style={{
          flex: 1,
          ...
          <div style={{ flex: 1, display: "flex", flexWrap: "wrap", gap: 2, alignContent: "flex-start", overflowY: "auto", minHeight: 0 }}>
            {discardPile.slice(-(compact ? 40 : 60)).map((t, i) => (
              <TileComponent key={i} tile={t} small dimmed={lastDiscard && t.id !== lastDiscard.id && i < discardPile.length - 1} mode={mode} />
            ))}
          </div>
          <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: 4, padding: "3px 6px", border: `1px solid ${COLORS.border}`, flexShrink: 0 }}>
            {gameLog.slice(-4).map((entry, i, arr) => (
              <div key={i} style={{ fontSize: 9, color: i === arr.length - 1 ? COLORS.text : COLORS.textMuted, lineHeight: 1.5 }}>
                {entry}
              </div>
            ))}
          </div>
        ...
        {/* North — scrollable vertically, wider to show revealed sets */}
        <div style={{ width: compact ? 40 : 62, flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, overflowY: "auto", overflowX: "hidden" }}>
          <div style={{ fontSize: 9, color: currentPlayer === 3 ? COLORS.goldLight : COLORS.textMuted, fontFamily: "'Cinzel', serif", writingMode: "vertical-rl", marginBottom: 2 }}>
            {currentPlayer === 3 && "◀ "}NORTH{flowers[3].length > 0 ? ` 🌸${flowers[3].length}` : ""}
          </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 1, justifyContent: "center" }}>
            {hands[3].filter(t=>!t.isFlower).slice(0, compact ? 4 : 5).map((_, i) => <TileComponent key={i} tile={_} faceDown small mode={mode} />)}
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 1, justifyContent: "center", marginTop: 2 }}>
            {discardPiles[3].slice(-3).map((t, i) => <TileComponent key={i} tile={t} small mode={mode} />)}
          </div>
        </div>
      </div>
        ...
        {claimOptions && !chowPicker && (
          <div style={{ background: "rgba(30,58,138,0.3)", border: "1px solid #3b6ea0", borderRadius: 6, padding: "7px 8px", marginBottom: 7, display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap" }}>
            <span style={{ fontSize: 11, color: "#93c5fd", fontFamily: "'Cinzel', serif" }}>
              CLAIM {tl(claimOptions.discardedTile)}?
            </span>
            {claimOptions.options.map((opt, i) => (
              ...
              >
                {opt.type === "win" ? "🏆 WIN!" : opt.type === "kong" ? "🀄 KONG" : opt.type === "pong" ? "PONG" : "CHOW"}
              </button>
            ))}
        ...
        {revealed[0].length > 0 && (
          <div style={{ display: "flex", gap: 5, marginBottom: 6, overflowX: "auto", paddingBottom: 2 }}>
            {revealed[0].map((set, si) => (
              <div key={si} style={{ display: "flex", gap: 2, flexShrink: 0, background: set.type === "kong" ? "rgba(167,139,250,0.15)" : "rgba(212,168,67,0.15)", padding: "3px 5px", borderRadius: 4, border: `1px solid ${set.type === "kong" ? "rgba(167,139,250,0.4)" : "rgba(212,168,67,0.3)"}` }}>
                <span style={{ fontSize: 8, color: set.type === "kong" ? "#c4b5fd" : COLORS.textMuted, alignSelf: "center", marginRight: 2 }}>
                  {set.type === "kong" ? (set.isConcealed ? "KONG🔒" : "KONG") : set.type.toUpperCase()}
                </span>
                {set.tiles.map((t, ti) => (
                  // Concealed kongs: middle two tiles shown face-down
                  <TileComponent key={ti} tile={t} small mode={mode}
                    faceDown={set.type === "kong" && set.isConcealed && (ti === 1 || ti === 2)} />
                ))}
              </div>
            ))}
          </div>
              ...
              <div key={tile.id} className="tile-hover">
                <TileComponent
                  ...
                  onClick={() => {
                    ...
                    if (drawnTile || totalHandCount > 16) {
        ...
        <div style={{ display: "flex", gap: 7, marginTop: 8, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {selectedTile && drawnTile && isMyTurn && !claimOptions && !chowPicker && (
            <button className="btn-gold" onClick={() => handleDiscard(selectedTile)} style={{ fontSize: 13, padding: "9px 16px" }}>
              Discard {tl(selectedTile)}
            </button>
          )}
          {canWinNow && !chowPicker && (
            <button className="win-btn" onClick={handleWin} style={{ fontSize: 14, padding: "9px 18px" }}>🏆 WIN!</button>
          )}
          {/* Concealed Kong buttons — one per available kong group */}
          {concealedKongOptions.map((kongTiles, i) => (
            <button key={i} className="claim-btn" onClick={() => handleConcealedKong(`${kongTiles[0].suit}-${kongTiles[0].value}`)}
              style={{ fontSize: 12, padding: "8px 14px", background: "linear-gradient(135deg, #1a2040, #2d1a4a)", borderColor: "#a78bfa", color: "#c4b5fd" }}>
              🀄 Kong ({tl(kongTiles[0])} ×4)
            </button>
          ))}
          {/* Upgrade Kong button — when drawn tile completes an exposed pong */}
          {upgradeKongOption && (
            <button className="claim-btn" onClick={() => handleUpgradeKong(`${upgradeKongOption.tiles[0].suit}-${upgradeKongOption.tiles[0].value}`)}
              style={{ fontSize: 12, padding: "8px 14px", background: "linear-gradient(135deg, #1a2040, #2d1a4a)", borderColor: "#a78bfa", color: "#c4b5fd" }}>
              🀄 Upgrade → Kong ({tl(upgradeKongOption.tiles[0])} ×4)
            </button>
          )}
    ...
    <div style={{ minHeight: "100dvh", background: COLORS.bg, color: COLORS.text, fontFamily: "'Noto Sans SC', 'Georgia', serif", display: "flex", flexDirection: "column" }}>
      ...
      <div className="sticky-header" style={{ borderBottom: `1px solid ${COLORS.border}`, background: COLORS.panel, padding: "8px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div className="header-title" style={{ fontFamily: "'Cinzel', serif", fontSize: 17, fontWeight: 600, color: COLORS.gold, letterSpacing: 2 }}>
              {mode === "cabrona" ? "TAIWANESE CABRONAHJONG, BITCHES!" : "TAIWANESE MAHJONG"}
            </div>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button
            onClick={() => setMode(m => m === "clean" ? "cabrona" : "clean")}
            style={{
              background: mode === "cabrona" ? "linear-gradient(135deg, #7c2d12, #dc2626)" : "transparent",
              color: mode === "cabrona" ? "white" : COLORS.gold,
              border: `1px solid ${mode === "cabrona" ? "#ef4444" : COLORS.gold}`,
              padding: "5px 10px", borderRadius: 4, cursor: "pointer",
              fontFamily: "'Cinzel', serif", fontSize: 10, letterSpacing: 0.5,
              transition: "all 0.2s", whiteSpace: "nowrap",
            }}
          >
            {mode === "clean" ? "😇 Clean" : "😈 Cabrona"}
          </button>
      ...
      {showRules && (
        <div style={{ background: "#071828", borderBottom: `1px solid ${COLORS.border}`, padding: "10px 14px", fontSize: 11, color: COLORS.textMuted, lineHeight: 1.8 }}>
          ...
          {mode === "cabrona" && <span style={{ color: "#ef4444" }}> 😈 Cabrona mode active — Bam = Pnis, Cir = Hole, Flwr = dFlw.</span>}&nbsp;
      ...
      <div className="desktop-layout" style={{ flex: 1 }}>
        <GameTable compact={false} mode={mode} />
      ...
      <div className="mobile-layout">
        {mobileTab === "game"
          ? <GameTable compact={true} mode={mode} />
          : (
