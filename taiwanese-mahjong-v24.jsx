...
Give specific, actionable advice. Keep responses under 180 words.`;
  ...
  const handleConcealedKong = useCallback((kongTiles) => {
    pushUndo(game, selectedTile);
    setGame(prev => {
      const newHand = prev.hands[0].filter(t => !kongTiles.some(k => k.id === t.id));
      // Kong set is stored with isConcealed:true — displayed face-down except the two outer tiles
      const withKong = {
        ...prev,
        hands: prev.hands.map((h,i) => i===0 ? newHand : h),
  ...
  const handleUpgradeKong = useCallback((pongSet) => {
  ...
  const GameTable = ({ compact, mode }) => (
    <div style={{
      ...
      <div style={{ display: "flex", gap: compact ? 4 : 8, alignItems: "stretch", height: compact ? 160 : 190 }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, display: "flex", flexWrap: "wrap", gap: 2, alignContent: "flex-start", overflowY: compact ? "auto" : "hidden", minHeight: 0 }}>
      ...
      <div style={{ background: "rgba(0,0,0,0.38)", borderRadius: 8, border: `1px solid ${COLORS.border}`, padding: compact ? "7px 8px" : "10px 14px" }}>
        ...
        <div style={{ display: "flex", gap: 7, marginTop: 8, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {concealedKongOptions.map((kongTiles, i) => (
            <button key={i} className="claim-btn" onClick={() => handleConcealedKong(kongTiles)}
          ...
          {upgradeKongOption && (
            <button className="claim-btn" onClick={() => handleUpgradeKong(upgradeKongOption)}
              style={{ fontSize: 12, padding: "8px 14px", background: "linear-gradient(135deg, #1a2040, #2d1a4a)", borderColor: "#a78bfa", color: "#c4b5fd" }}>
