...
function analyzeHand(hand, discardPile, revealedSets, allPlayerDiscards) {
  ...
  for (const suit of ["man","bam","circle"]) {
    const tiles = (suitTiles[suit] || []).filter(t => !tripletIds.has(t.id) && !pairIds.has(t.id));
    for (let i = 0; i < tiles.length; i++) {
  ...
  const usedInComplete = new Set([...tripletIds, ...pairIds, ...sequenceIds]);
  const partials = [];
  const partialIds = new Set();
  const freeSuitTiles = playable.filter(t => ["man","bam","circle"].includes(t.suit) && !usedInComplete.has(t.id));
  for (let i = 0; i < freeSuitTiles.length; i++) {
    if (partialIds.has(freeSuitTiles[i].id)) continue;
    for (let j = i + 1; j < freeSuitTiles.length; j++) {
      if (partialIds.has(freeSuitTiles[j].id)) continue;
      const a = freeSuitTiles[i], b = freeSuitTiles[j];
  ...
  // ── ISOLATED: nothing useful at all ─────────────────────────────────────
  const allUsefulIds = new Set([...tripletIds, ...pairIds, ...sequenceIds, ...partialIds]);
  const isolated = playable.filter(t => !allUsefulIds.has(t.id));
  ...
  // Coaching fires whenever it's your turn (16 tiles before draw, 17 after draw)
  ...
  if (needToDiscard) {
    if (safeDiscards.length > 0) {
      ...
      const honorIsolated = isolated.filter(t => t.suit === "wind" || t.suit === "dragon");
      const edgeIsolated = isolated.filter(t => ["man","bam","circle"].includes(t.suit) && (t.value === 1 || t.value === 9));
      const midIsolated = isolated.filter(t => ["man","bam","circle"].includes(t.suit) && t.value >= 2 && t.value <= 8);
      if (honorIsolated.length > 0) {
        bestDiscard = honorIsolated[0];
        reasoning = `${tileLabel(bestDiscard)} is isolated — no pair, triplet, or sequence connections. Honor tiles (winds/dragons) can only form triplets, not sequences, making them risky to hold without a pair.`;
      } else if (edgeIsolated.length > 0) {
        bestDiscard = edgeIsolated[0];
        reasoning = `${tileLabel(bestDiscard)} is an isolated edge tile. Ones and nines connect on only one side, so they're harder to complete into sequences than middle tiles.`;
      } else {
        bestDiscard = midIsolated[0] || isolated[0];
        reasoning = `${tileLabel(bestDiscard)} has no useful connections — no pair, no sequence potential. Shed it to improve your hand shape.`;
      }
    } else if (partials.length > 0) {
      // All tiles have some connection. Find the weakest partial to break.
      // Prefer breaking a partial whose needed tile is already gone from the discard pile
      const deadPartial = partials.find(p => p.need.every(n => {
        const key = `${n.suit}-${n.value}`;
        return (discardPile.filter(d => `${d.suit}-${d.value}` === key).length >= 4);
      }));
      if (deadPartial) {
        // Pick the more isolated of the two tiles (edge > honor > mid)
        const candidates = deadPartial.tiles.filter(t => t.suit !== "man" && t.suit !== "bam" && t.suit !== "circle"
          ? true : t.value === 1 || t.value === 9);
        bestDiscard = candidates[0] || deadPartial.tiles[0];
        ...
        // Break a partial that's least connected to the rest of the hand
        // Pick edge tile from a partial if possible
        const edgeTileFromPartial = partials.flatMap(p => p.tiles).find(t => ["man","bam","circle"].includes(t.suit) && (t.value === 1 || t.value === 9));
        if (edgeTileFromPartial) {
          bestDiscard = edgeTileFromPartial;
          reasoning = `${tileLabel(bestDiscard)} is part of a partial sequence but sits at the edge of a suit. It connects in only one direction, making it weaker than your other partials. Consider releasing it if you have stronger paths to winning.`;
        } else {
          bestDiscard = partials[partials.length - 1].tiles[0];
          reasoning = `${tileLabel(bestDiscard)} is your weakest partial connection. Your hand has good shape overall — this is a judgment call about which suit to commit to.`;
        }
      }
    } else if (pairs.length > 0 && triplets.length === 0) {
      // Hand is dense with pairs and sequences — recommend weakest pair as possible discard only if overstuffed
      ...
      if (honorPair) {
        ...
        bestDiscard = null; // No clear recommendation — hand is strong
      }
    } else {
...
export default function MahjongApp() {
  ...
  const CoachPanel = () => (
    <div style={{ flex: 1, overflow: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {/* ── WAITING FOR DRAW — no advice yet ── */}
      {!claimAdvice && isMyTurn && !drawnTile && (
      ...
      {/* ── POST-DRAW DISCARD ADVICE ── */}
      {coaching && isMyTurn && drawnTile && (
        <>
  ...
  const GameTable = ({ compact }) => (
    <div style={{
      ...
      {/* Opponent at top (West) — strictly one row, no wrapping */}
      <div style={{ flexShrink: 0, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
        ...
        {/* Tiles row: overflow scrolls horizontally, NEVER grows taller */}
        <div style={{ display: "flex", gap: 2, overflowX: "auto", overflowY: "hidden", width: "100%", paddingBottom: 2, flexShrink: 0, flexWrap: "nowrap" }}>
        ...
        {/* Discards row: strictly limited, no wrap */}
      ...
      {/* Middle row — flex:1 but with minHeight so it doesn't collapse */}
      <div style={{ flex: 1, minHeight: compact ? 110 : 160, display: "flex", gap: compact ? 4 : 10 }}>
        {/* South */}
        <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2, width: compact ? 36 : 52 }}>
          <div style={{ fontSize: 9, color: currentPlayer === 1 ? COLORS.goldLight : COLORS.textMuted, fontFamily: "'Cinzel', serif", writingMode: "vertical-rl" }}>
          ...
          {hands[1].slice(0, compact ? 3 : 4).map((_, i) => <TileComponent key={i} tile={_} faceDown small />)}
          {discardPiles[1].slice(-2).map((t, i) => <TileComponent key={i} tile={t} small />)}
        </div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, display: "flex", flexWrap: "wrap", gap: 2, alignContent: "flex-start", overflow: "auto" }}>
            {discardPile.slice(-(compact ? 18 : 28)).map((t, i) => (
              <TileComponent key={i} tile={t} small dimmed={lastDiscard && t.id !== lastDiscard.id && i < discardPile.length - 1} />
            ))}
          </div>
          <div style={{ flexShrink: 0, background: "rgba(0,0,0,0.3)", borderRadius: 4, padding: "4px 6px", border: `1px solid ${COLORS.border}` }}>
            {gameLog.slice(-2).map((log, i) => (
              <div key={i} style={{ fontSize: 10, color: i === gameLog.slice(-2).length - 1 ? COLORS.text : COLORS.textMuted, lineHeight: 1.4 }}>{log}</div>
            ))}
          </div>
        </div>
        ...
        {/* North */}
        <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2, width: compact ? 36 : 52 }}>
          <div style={{ fontSize: 9, color: currentPlayer === 3 ? COLORS.goldLight : COLORS.textMuted, fontFamily: "'Cinzel', serif", writingMode: "vertical-rl" }}>
          ...
          {hands[3].slice(0, compact ? 3 : 4).map((_, i) => <TileComponent key={i} tile={_} faceDown small />)}
          {discardPiles[3].slice(-2).map((t, i) => <TileComponent key={i} tile={t} small />)}
        </div>
      </div>

      {/* Player hand area — flexShrink:0 so it NEVER gets cut off */}
      <div style={{ flexShrink: 0, background: "rgba(0,0,0,0.38)", borderRadius: 8, border: `1px solid ${COLORS.border}`, padding: compact ? "7px 8px" : "10px 14px" }}>
  ...
  return (
    <div style={{ minHeight: "100dvh", background: COLORS.bg, color: COLORS.text, fontFamily: "'Noto Sans SC', 'Georgia', serif", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media (max-width: 700px) {
          ...
          .mobile-layout { display: flex !important; flex-direction: column; flex: 1; overflow: hidden; min-height: 0; }
      ...
      {/* ── HEADER ──────────────────────────────────────────────────────── */}
      <div style={{ borderBottom: `1px solid ${COLORS.border}`, background: COLORS.panel, padding: "8px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
      ...
      {showRules && (
        <div style={{ background: "#071828", borderBottom: `1px solid ${COLORS.border}`, padding: "10px 14px", fontSize: 11, color: COLORS.textMuted, lineHeight: 1.8 }}>
          <strong style={{ color: COLORS.gold }}>Rules:</strong> 4 sets + 1 pair to win. 16 tiles each. Chow (sequence) or Pong (triplet) from any player. Flowers replaced automatically. Win by self-draw or claiming a discard. Special: All Pairs.&nbsp;
      ...
      <div className="desktop-layout" style={{ flex: 1, overflow: "hidden", minHeight: 0 }}>
        ...
        <div style={{ width: 280, background: COLORS.panel, borderLeft: `1px solid ${COLORS.border}`, display: "flex", flexDirection: "column", overflow: "hidden", flexShrink: 0 }}>
      ...
      <div className="mobile-layout">
        ...
        <div style={{ flex: 1, overflow: "hidden", minHeight: 0, display: "flex", flexDirection: "column" }}>
          {mobileTab === "game"
            ...
            : (
              <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column", background: COLORS.panel }}>
        ...
        {/* Bottom tab bar */}
        <div className="mobile-tabbar" style={{ background: COLORS.panel, borderTop: `1px solid ${COLORS.border}`, flexShrink: 0, paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
          {[
